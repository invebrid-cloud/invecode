...
function NotificationsSkeleton() {
  return (
    <div className="space-y-4">
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className="flex items-start gap-4 rounded-lg border p-4 bg-muted/40 animate-pulse">
...
export default function NotificationsPage() {
  ...
  const markAsRead = async (id: number | string) => {
    // Optimistically update context & query cache
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              notifications.map((notification, index) => (
                <Dialog key={notification.id && !Number.isNaN(notification.id) ? notification.id : index}>
